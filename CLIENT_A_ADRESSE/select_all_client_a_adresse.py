# select_all_client_a_adresse.py
...
try:
    ...
    mysql_select_string = ("SELECT id_adresse, RueAdresse , NpaAdresse , VilleAdresse ,  id_client, NomClient, PrenomClient , FROM t_client_a_adresse AS T1\n"
                           "INNER JOIN t_client AS T2 ON T2.id_client = T1.fk_client\n"
                           "INNER JOIN t_adresse AS T3 ON T3.id_adresse = T1.fk_adresse")
    ...
    for row in records_select:
        print(row['NomClient'],row['RueAdresse , NpaAdresse , VilleAdresse'],)

    for row in records_select:
        output = "nom du client: {NomClient}  Adresse: {RueAdresse , NpaAdresse , VilleAdresse}"
