...
try:
    ...
    mysql_select_string = "SELECT * FROM t_adresse ORDER BY id_adresse ASC"
    ...
    for row in records_select:
        print(row['id_adresse'],row['RueAdresse'],row['NpaAdresse'],row['VilleAdresse'])

    for row in records_select:
        output = "id : {id_adresse}  Rue : {RueAdresse}  Npa : {NpaAdresse}  Ville : {VilleAdresse}"
