...
try:
    ...
    mysql_select_string = "SELECT * FROM t_client ORDER BY id_client ASC"
    ...
    for row in records_select:
        print(row['id_client'],row['NomClient'],row['PrenomClient'])

    for row in records_select:
        output = "id: {id_client}  nom: {NomClient}  prenom{PrenomClient}"
